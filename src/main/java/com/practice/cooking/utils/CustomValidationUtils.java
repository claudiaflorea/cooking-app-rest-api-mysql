...
public class CustomValidationUtils {
    ...
    public void validateSmoothieRecipe(RecipeDto recipeDto, Errors errors) {
        // recipe must contain 0.5 liters of milk
        // recipe must contain at least one fruit
        // recipe type must be beverage

        List<String> fruits = asList("Apple", "Orange", "Banana", "Strawberry", "Berry", "Raspberry");
        int hasFruits = 0;
        int hasMilk = 0;
        ...
        if (recipeDto.getIngredients() != null && recipeDto.getIngredients().size() > 0) {
            ...
            for (IngredientDto ingredientDto : recipeDto.getIngredients()) {
                if (fruits.contains(ingredientDto.getName())) {
                    hasFruits++;
                }
                if (ingredientDto.getUnit() == Unit.LITER && ingredientDto.getName().equals("Milk") && ingredientDto.getQuantity() == 0.5) {
                    hasMilk++;
                }
            }
            if (hasFruits == 0) {
                errors.rejectValue("name", DTO_RECIPE_NAME, DTO_RECIPE_NAME + SEPARATOR + "  must contain at least one fruit");
            }
            if (hasMilk == 0) {
                errors.rejectValue("name", DTO_RECIPE_NAME, DTO_RECIPE_NAME + SEPARATOR + "  must contain exactly 0.5 liters of milk");
            }
            if (recipeDto.getRecipeType() != RecipeType.BEVERAGE) {
                errors.rejectValue("name", DTO_RECIPE_NAME, DTO_RECIPE_NAME + SEPARATOR + "  must be a beverage");
            }
        }
    }
}
